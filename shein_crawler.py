...
class SheinCrawler():
    ...
    def get_repeated_classes(self, soup) -> dict:
    ...
    def get_element_with_repeated_clases(self, repeated_classes: dict, filtered_data: bs4.element.ResultSet):
    ...
    def extract_text(self, code: str) -> str:
        '''
    ...
    def filter_data(self, soup) -> dict:
        # FILTERING STEP 1. "Getting the divs"
        filtered_data = soup.find_all("div", attrs={"class":"product-list"})
        repeated_classes = self.get_repeated_classes(soup=filtered_data)
        elements = self.get_element_with_repeated_clases(repeated_classes, filtered_data)
        ...
        for key, value in elements.items():
            ...
            for i in range(len(value)):
                arr.append(self.extract_text(value[i]))
            elements[key] = arr
        return elements

    def save_to_csv(self, elements: pd.DataFrame, date: str) -> None:
